...
public sealed class AnalyzerTests : IDisposable
{
    ...
    public AnalyzerTests()
    {
        ...
        CreateMod("FullyOn", "installationFile=Alpha-1234.7z");
        ...
        // Profiles: Alpha is enabled in Main but disabled in Variant, so its
        // AlwaysEnabled tag still matters; FullyOn is enabled everywhere it appears.
        ...
        var variantDir = Path.Combine(_source, "profiles", "Variant");
        Directory.CreateDirectory(variantDir);
        File.WriteAllText(Path.Combine(variantDir, "modlist.txt"),
            "# This file was automatically generated by Mod Organizer.\r\n" +
            "-Alpha\r\n");
    ...
    {
        Profile = "Main",
        AdditionalProfiles = ["Variant"],
        Ignore = ["logs", "mods\\LongGoneMod"],
        AlwaysEnabled = ["mods\\Optional AE", "mods\\Alpha", "mods\\FullyOn"],
    ...
    public void Analyze_Finds_All_Three_Categories()
    {
        ...
        Assert.Equal(["Variant", "Main"], result.ProfilesUsed);

        // Stale (dead references only): missing Ignore entry, duplicate NoMatchInclude entry.
        ...
        Assert.Equal(2, result.StaleEntries.Count);

        // Redundant AlwaysEnabled: only FullyOn (enabled everywhere it is listed).
        // Alpha is disabled in Variant, where the tag preserves the '-' line, so it
        // must NOT be reported.
        var redundant = Assert.Single(result.RedundantAlwaysEnabled);
        Assert.Equal("mods\\FullyOn", redundant.Entry);
        Assert.Equal(StaleEntryKind.RedundantAlwaysEnabled, redundant.Kind);
    ...
    public void Updater_Applies_Decisions_And_RoundTrips()
    {
        ...
        plan.RemoveEntries.AddRange(result.StaleEntries);           // both dead references
        plan.RemoveEntries.AddRange(result.RedundantAlwaysEnabled); // FullyOn's inert tag
        ...
        Assert.Equal(["mods\\Optional AE", "mods\\Alpha", "mods\\Beta"], settings.AlwaysEnabled);
